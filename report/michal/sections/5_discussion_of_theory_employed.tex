...
A \textit{group} is a set of objects $G$ equipped with a binary operation (here denoted $\cdot$ and referred to as \textit{multiplication}) which satisfies the following axioms \cite[Ch. 1]{dresselhaus}:
\begin{enumerate}
...
\end{enumerate}
A \textit{representation} $\Gamma$ (rep) is a mapping of group elements $g$ onto matrices $D^{(\Gamma)}(g)$ that preserves the group operation as matrix multiplication \cite[Ch. 2]{dresselhaus}:
\begin{equation}
